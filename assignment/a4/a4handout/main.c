...
///// REPLACE THIS /////
    ...
    int i = 0;
    do{
        if (L1_cache[getL1SetID(address)][a].timeStamp > L1_cache[getL1SetID(address)][i].timeStamp  ){a = i;
        }
    ...
    int j = 0;
    do{
        if(L2_cache[getL2SetID(address)][j].timeStamp < L2_cache[getL2SetID(address)][b].timeStamp){b = j;}
