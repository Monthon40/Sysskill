...
int is_in_range(life_board board, int row, int col)
{
    ...
    if( board.num_cols > col && board.num_rows > row){
        return 1;
    }
    return 0;
}
...
{
    int ttt = 0;
    if(is_alive(board,row+1,col)){ttt++;}

    if(is_alive(board,row-1,col)){ttt++;}

    if(is_alive(board,row,col+1)){ttt++;}

    if(is_alive(board,row,col-1)){ttt++;}

    if(is_alive(board,row+1,col+1)){ttt++;}
...
{
    int aa = 0;
    while ( aa < current.num_rows){
//    for(int aa = 0; aa < current.num_rows ; aa++){
        for(int ii = 0; ii < current.num_cols; ii++){

            if(4 > count_live_nbrs(current,aa,ii) && count_live_nbrs(current,aa,ii) >= 2){
//            if(count_live_nbrs(current, aa, ii) >= 2 && count_live_nbrs(current, aa, ii) < 4){
                set_alive(next, aa, ii);
            }

            if( 2 > count_live_nbrs(current, aa, ii)){
                set_dead(next, aa, ii);
            }
            if( 4 < count_live_nbrs(current, aa, ii)){
                set_dead(next, aa, ii);
            }
            if(!is_alive(current, aa, ii)){
                if(count_live_nbrs(current, aa, ii) == 3){
                    set_alive(next, aa, ii);
                }else{
                    set_dead(next, aa, ii);
                }
